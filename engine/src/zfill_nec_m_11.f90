...
!------------------------------------------------------------------------------
   type :: NEC_ZFILL_TYPE
      integer :: selfKernel = KERNEL_EXACT   ! RTWK, ETWK, or EXACT for the
                                              ! self term. Default changed to
                                              ! EXACT: ETWK validated to
                                              ! underestimate by up to ~17%
                                              ! when rho~a (see module header
                                              ! and exact_core). Time is not
                                              ! a constraint per instruction;
                                              ! correctness is.
      integer :: nearKernel = KERNEL_EXACT   ! ETWK or EXACT for
                                              ! isNearTouching pairs
                                              ! (source_pair_etwk vs
                                              ! source_pair_exact). Same
                                              ! reasoning as selfKernel.
                                              ! RTWK is not a valid value
                                              ! here (isNearTouching pairs
                                              ! never used plain RTWK
                                              ! quadrature).

      ! Hybrid NEAR classification thresholds (see classify_near). Node-
      ! sharing is always NEAR regardless of these. Defaults are starting
      ! points, not validated against your mesh conventions -- tune them.
      real :: C1_lenMult = 3.0    ! centroid sep < C1_lenMult * max(Lp,Lq)
      real :: C2_lambda  = 0.25   ! centroid sep < C2_lambda * lambda
      real :: C3_radMult = 5.0    ! axis-to-axis gap < C3_radMult * wireRadius
   contains
   ...
   function Z_half_pair_nec(this, Hp, Hq, wireRadius, Segs, Bk, useImage, Refl) result(Z)
      ...
      do ip = 1, nQtest
         ...
         if (isSelf) then
            ...
            ! Node-sharing or true surface proximity: field point's axial
            ! projection is at/near the source segment's own span. Default
            ! is now the exact double-quadrature kernel (source_pair_exact)
            ! -- ETWK (source_pair_etwk) validated to underestimate by up to
            ! ~17% when rho~a; kept available via nearKernel for comparison.
            ...
            if (this%nearKernel == KERNEL_EXACT) then
