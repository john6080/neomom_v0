...
!------------------------------------------------------------------------------
   type :: NEC_ZFILL_TYPE
     ! integer :: selfKernel = KERNEL_ETWK   ! RTWK or ETWK for the self term
      integer :: selfKernel = KERNEL_RTWK   ! RTWK or ETWK for the self term
      ...
      real :: C3_radMult = 0.0 !2.0 !5.0    ! axis-to-axis gap < C3_radMult * wireRadius
   contains
   ...
   function Z_half_pair_nec(this, Hp, Hq, wireRadius, Segs, Bk, useImage, Refl) result(Z)
      ...
      do ip = 1, nQtest
         ...
         if (isSelf) then
            ...
            ! projection is at/near the source segment's own span --
            ! ETWK's closed-form treatment is appropriate here.
