...
!
!  Sweep mode selected by sys%sweep_mode ('pattern' or 'vna_sweep'):
!
...
!  end do
!
!  VNA SWEEP mode (sweep=vna_sweep or OPTIONS sweep_mode='vna_sweep'):
!  open _Zin.csv
!  do iFreq = 1, sys%freq%nFreq
!   ├─ Steps 1-11: mesh, solve, Zin, SWR (always run)
...
!==============================================================================
   ...
   if (trim(sys%sweep_mode) == 'vna_sweep') call open_Zin_csv(sys, iZin)
   ...
   if (trim(sys%sweep_mode) == 'vna_sweep') call close_Zin_csv(sys, iZin)
...
!
!  Called once before the frequency loop when sweep_mode = 'vna_sweep'.
...
!
!  Called once per frequency inside the loop when sweep_mode = 'vna_sweep'.
...
!
!  Called once after the frequency loop when sweep_mode = 'vna_sweep'.
