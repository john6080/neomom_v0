...
!------------------------------------------------------------------------------
   type :: NEC_ZFILL_TYPE
    !  integer :: selfKernel = KERNEL_EXACT   ! RTWK, ETWK, or EXACT for the
      integer :: selfKernel = KERNEL_RTWK                              ! self term. Default changed to
     ...
     ! integer :: nearKernel = KERNEL_EXACT   ! ETWK or EXACT for
                                              ...
                                              ! (source_pair_etwk vs
      integer :: nearKernel =   KERNEL_BARE                 ! source_pair_exact), OR
                                              ! KERNEL_RTWK to route them
                                              ! through source_gauss_nec
                                              ! instead (simple filament +
                                              ! a^2 floor, NO phi-averaging)
                                              ! -- see the note at its use
                                              ! in Z_half_pair_nec. Added
                                              ! after EXACT barely moved
                                              ! Zin vs ETWK on the shorted-
                                              ! TL test (292.7->271.5),
                                              ! suggesting the phi-averaged
                                              ! model itself (not its
                                              ! accuracy) may be the wrong
                                              ! generalization for PAIR
                                              ! (non-self) interactions --
                                              ! Burke only validates
                                              ! phi-averaging for the self
                                              ! term.
      ...
      real :: C1_lenMult = 0.0 !3.0    ! centroid sep < C1_lenMult * max(Lp,Lq)
      real :: C2_lambda  = 0.0 !0.25   ! centroid sep < C2_lambda * lambda
      real :: C3_radMult = 0.0 !5.0    ! axis-to-axis gap < C3_radMult * wireRadius
   contains
